...
\begin{document}
	...
	\begin{story}[forest-elfnature-warrior]{대자연의 전사}{[종족: 숲 엘프][기피][공포]\hyperlink{celesteela}{ⓣ}}
		\pre{거주하던 자연환경 하나를 미리 선택한다.}
		
		\flavour{자연 속에서 거주하며, 세상 물정에 어둡다.}
		
		\limitedtrauma{기피}{도시 출신 인물들과 대화할 때, 세상 물정에 어둡기 때문에 관련 지식과 인맥 판정에 -1을 받는다.}
		
		\triggertrauma{공포}{무차별적인 자연 파괴 현장을 목격한다.}{1d6을 굴려, 다음을 적용시킨다:
		
		\begin{tightcenter}
			\begin{tabularx}{\linewidth}{c|X}
				\textbf{주사위} & \makecell{\centering\textbf{효과}}                                        \\\hline\hline
	...
	\begin{story}{향수병}{[종족: 숲 엘프][트라우마: 가변]\hyperlink{celesteela}{ⓣ}}
		...
		\triggertrauma{}{\storyref{forest-elf:nature-warrior}{대자연의 전사}에서 정한 자연환경 또는 [태초의 이야기]에 가지 않는 날이 3일 이상 지속된다.}
		...
		\entry[\hline]{해당 자연환경에 연관된 사진, 물체 등을 접함으로서 일시적으로 한 씬 동안 효과를 한 단계 완화시킬 수 있다.}
	\end{story}
	
	\begin{story}{날랜 추적자}{[종족: 숲 엘프]\hyperlink{celesteela}{ⓣ}}
		\entry{\statchange{+}{기민}}
		
		\entry{\statchange{-}{근력, (기민을 제외한 특화되지 않은 스탯 중 하나 선택)}}
