...
\begin{document}
	...
	\begin{story}[dice_blackjack]{다이스 블랙잭}{[클래스: 도박사]}
		매 턴 한번, 같은 구역에 있는 사람을 하나 지목해, \storyref{dice_blackjack}{다이스 블랙잭}을 진행할 수 있다:
		\begin{enumerate}
	...
	\begin{story}{도박 중독}{[클래스: 도박사][중독]}
		\textbf{제약(중독)}: 다른 이에게 피해를 가하고 싶을 때 \storyref{dice_blackjack}{다이스 블랙잭}을 할 수 있다면, 반드시 \storyref{dice_blackjack}{다이스 블랙잭}을 통해서만 피해를 줄 수 있다.
	...
	\begin{story}{올인}{[클래스: 도박사][집착]}
		\storyref{dice_blackjack}{다이스 블랙잭}의 결과가 \textcolor{red}{[스페셜]}일 때, 즉시 [멍해짐: 1턴] 상태를 주는 것으로 게임으로 인한 피해량 판정 없이 게임을 종료할 수 있다. [멍해짐] 상태를 받은 상대는 다음 자신의 턴에 아무 행동도 할 수 없다.
		
		\smallskip
		
		\textbf{제약(집착)}: \storyref{dice_blackjack}{다이스 블랙잭}의 결과가 \textcolor{red}{[스페셜]}이라도, \textcolor{green}{[스플릿]} 할 수 없다.
		
		\cost{10}
	\end{story}
	
	\begin{story}{밑장빼기}{[클래스: 도박사][기피]}
		전투 씬에서, 세 라운드에 한 번, \storyref{dice_blackjack}{다이스 블랙잭}을 할때 자신이 \textcolor{blue}{[버스트]}가 난 경우, \textcolor{blue}{[버스트]}를 낸 주사위 굴림을 취소할 수 있다.
		...
		\textbf{트리거(기피)}: \storyref{dice_blackjack}{다이스 블랙잭}을 할때, 자신이 주사위를 더 굴리지 않겠다고 선언하지 않았고, 상대가 \textcolor{red}{[스페셜]}이다.
